...
8. `0008_recompute_bonuses_idempotent.sql` — fixes the bonus-correction bug from the Phase 4 doc-audit. `recompute_bonuses` now DELETEs every bonus row in its scope (`group_*` and `champion`) before re-INSERTing the currently-correct ones, so an admin re-entering a wrong score no longer leaves stale awards behind.
9. `0009_realtime.sql` — opts the `score` and `bonus` tables into the `supabase_realtime` publication so the live ranking page receives `postgres_changes` events.
